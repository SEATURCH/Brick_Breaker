...
void DrawBallObjectMovement(
    ...
    int L1, L2, R1, R2, offset, temp_moveX, temp_moveY, temp_index, temp1 = 0, temp2;
    int potential_L1_index, potential_L2_index, potential_R1_index, potential_R2_index, old_L1_index;
    ...
    *spi_read = 0;
    while (1) {
       ...
       // MovePaddle(paddle, position); // use it with IR sensor
        ...
        if(cursorY == (INITIAL_PADDLE_Y_POS - BALL_OBJECT_HEIGHT) && cursorX < (paddle->x_pos + DEFAULT_PADDLE_WIDTH) && cursorX >= (paddle->x_pos - BALL_OBJECT_WIDTH)){
