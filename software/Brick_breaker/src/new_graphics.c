...
void DrawBallObjectMovement(BlockObjectStructure *blockObjectStructure, RenderObjectStructure *renderObjectStructure) {
    int cursorY = 72;
    int cursorX = 72;
    ...
    int blockCursor, L1, L2, R1, R2, offset, temp_moveX, temp_moveY;
    int index, potential_L1_index, potential_L2_index, potential_R1_index, potential_R2_index, old_L1_index, change_dir = 0;
    ...
    while (1) {
        ...
        cursorX += (Xspeed * Xdir);
        cursorY += (Yspeed * Ydir);
        if (cursorY <= BLOCK_AREA_LIMIT && cursorY >= 0 && cursorX <= X_LIMIT && cursorX >= 0) {
            // check for collision
            //draw? - NO
            //define new potential position

            potential_L1_index = ((cursorY/8) * (40)) + (cursorX/8);
            potential_L2_index = (((cursorY + BALL_OBJECT_HEIGHT)/8) * (40)) + (cursorX/8);
            potential_R1_index = ((cursorY/8) * (40)) + ((cursorX + BALL_OBJECT_WIDTH)/8);
            potential_R2_index = (((cursorY + BALL_OBJECT_HEIGHT)/8) * (40)) + ((cursorX + BALL_OBJECT_WIDTH)/8);
            // not dealing with x = 0 or y = 0 yet
            if(Xdir > 0 && Ydir > 0) offset = 41;
            if(Xdir < 0 && Ydir > 0) offset = 39;
            if(Xdir > 0 && Ydir < 0) offset = -39;
            if(Xdir < 0 && Ydir < 0) offset = -41;

            if(potential_R2_index >= 0 && potential_R2_index < NUM_RENDER_OBJECTS_TOTAL){
                R2 =  DetectCollision(potential_R2_index, oldCursorX, oldCursorY, offset, renderObjectStructure, blockObjectStructure);
            }

            if(potential_L2_index >= 0 && potential_L2_index < NUM_RENDER_OBJECTS_TOTAL){
                L2 =  DetectCollision(potential_L2_index, oldCursorX, oldCursorY, offset, renderObjectStructure, blockObjectStructure);
            }

            if(potential_R1_index >= 0 && potential_R1_index < NUM_RENDER_OBJECTS_TOTAL){
                R1 =  DetectCollision(potential_R1_index, oldCursorX, oldCursorY, offset, renderObjectStructure, blockObjectStructure);
            }

            if (L2 > -1 ){
                blockObject = &blockObjectStructure->blockObjects[L2];
                temp_moveY = (blockObject->blockYStart * RENDER_OBJECT_HEIGHT) - (cursorY + BALL_OBJECT_HEIGHT);
                temp_moveX = temp_moveY;
            }
            if (R1 > -1 ){
                blockObject = &blockObjectStructure->blockObjects[R1];
                temp_moveX = (blockObject->blockXStart * RENDER_OBJECT_WIDTH) - (cursorX + BALL_OBJECT_WIDTH);
            ...
            if (collision){
                ...
                EraseBlockAndResetRenderObjects(L2, blockObjectStructure, renderObjectStructure);
                EraseBlockAndResetRenderObjects(R1, blockObjectStructure, renderObjectStructure);
                EraseBlockAndResetRenderObjects(R2, blockObjectStructure, renderObjectStructure);
            }
    }
}
...
int DetectCollision(
	...
	while((blockCursor != -1) &&
			(Xstart > oldCursorX ||
					Ystart > oldCursorY)){ // ther's a block and condition met
...
void EraseBlockAndResetRenderObjects(
	...
	if(blockCursor > -1){
		...
		if(blockObject->isActive == FALSE) return;
		int renderObjectWidth = blockObject->blockWidthRenderObjects;
		int renderObjectHeight = blockObject->blockHeightRenderObjects;
		int renderObjectColor = (int) blockObject->blockType;
		int blockXStart = blockObject->blockXStart;
